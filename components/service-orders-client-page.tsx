...
import type { ServiceOrderRow } from "@/lib/data/service-orders"
import { generateFromServiceOrder } from "@/app/actions/orders-to-cash"
import { Card, CardContent } from "@/components/ui/card"
import { Input } from "@/components/ui/input"
import { Wrench, Plus, Search, AlertTriangle, SendHorizontal, CheckCircle2, Clock, FileDown, Pencil, Ban, X } from "lucide-react"
...
const processLabels: Record<string, string> = {
  ...
  BORDADO: "Bordado", ACABADO: "Acabado", EMPAQUE: "Empaque", LAVADO: "Lavado", OTROS: "Otros",
}
...
function CajaCell({ row, isDemo, onSend }: { row: ServiceOrderRow; isDemo: boolean; onSend: (id: string) => void }) {
...
export function ServiceOrdersClientPage({
  orders,
  isDemo,
}: {
  ...
  const filtered = orders.filter((os) => {
    ...
    return (
      os.orderNumber.toLowerCase().includes(q) ||
      os.supplier.toLowerCase().includes(q) ||
      os.process.toLowerCase().includes(q) ||
      (os.proformaCode?.toLowerCase().includes(q) ?? false) ||
      os.responsible.toLowerCase().includes(q)
    )
  })

  const totalPendiente = filtered
    .filter((os) => os.paymentStatus !== "PAGADO")
    .reduce((acc, os) => acc + os.pendingAmount, 0)

  const handleCancelConfirm = () => {
    if (!cancelTarget) return
    setCancelError(null)
    startCancelTransition(async () => {
      const result = await cancelServiceOrder(cancelTarget.id, cancelReason)
      if ("error" in result) {
  ...
  const handleSendToCash = (orderId: string) => {
    ...
    startTransition(async () => {
      const result = await generateFromServiceOrder(orderId)
      setPendingId(null)
      if ("error" in result) {
  ...
  return (
    <div className="flex flex-col min-h-full">
      <Header title="Órdenes de Servicio" subtitle="Servicios de talleres y terceros" />
      ...
      <main className="flex-1 p-6 space-y-4">
        ...
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div className="relative w-64">
            ...
            <Input
              placeholder="Buscar OS, taller, proceso, proforma..."
          ...
          <CardContent className="p-0">
            {filtered.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-16 gap-2 text-muted-foreground">
                ...
                <p className="text-sm">No se encontraron órdenes de servicio</p>
              </div>
            ) : (
                ...
                <table className="w-full text-xs">
                  <thead className="border-b border-border">
                    <tr className="bg-muted/40">
                      <th className="px-4 py-3 text-left font-semibold text-muted-foreground">N° OS</th>
                      <th className="px-4 py-3 text-left font-semibold text-muted-foreground">Fecha</th>
                      <th className="px-4 py-3 text-left font-semibold text-muted-foreground">Taller / Proveedor</th>
                      <th className="px-4 py-3 text-left font-semibold text-muted-foreground">Proceso</th>
                      <th className="px-4 py-3 text-left font-semibold text-muted-foreground">Proforma</th>
                  ...
                  <tbody className="divide-y divide-border">
                    {filtered.map((os) => {
                      const st = statusConfig[os.status] ?? { label: os.status, className: "bg-muted text-muted-foreground" }
                      const pt = paymentConfig[os.paymentStatus] ?? { label: os.paymentStatus, className: "bg-muted text-muted-foreground border border-border" }
                      const isSending = isPending && pendingId === os.id
                      return (
                        <tr key={os.id} className={`hover:bg-muted/30 transition-colors ${isSending ? "opacity-60" : ""}`}>
                          <td className="px-4 py-3 font-mono font-medium text-primary">{os.orderNumber}</td>
                          <td className="px-4 py-3 text-muted-foreground whitespace-nowrap">{fmtDate(os.issueDate)}</td>
                          <td className="px-4 py-3 text-foreground max-w-[160px] truncate">{os.supplier}</td>
                          <td className="px-4 py-3">
                            <span className="inline-block px-2 py-0.5 rounded-full text-[10px] font-medium bg-primary/10 text-primary">
                              {processLabels[os.process] ?? os.process}
                            </span>
                          </td>
                          <td className="px-4 py-3 font-mono text-muted-foreground text-[11px]">{os.proformaCode ?? "—"}</td>
                          <td className="px-4 py-3 text-right tabular-nums font-medium text-foreground">{fmt(os.totalAmount)}</td>
                          <td className={`px-4 py-3 text-right tabular-nums font-medium ${os.pendingAmount > 0 ? "text-amber-700" : "text-muted-foreground"}`}>
                            {os.pendingAmount > 0 ? fmt(os.pendingAmount) : "—"}
                          </td>
                          <td className="px-4 py-3">
                          ...
                          <td className="px-4 py-3">
                            {!isDemo && os.status !== "ANULADA" && (
                              <div className="flex items-center gap-1.5">
                                <Link
                                  href={`/service-orders/${os.id}/edit`}
                                ...
                                {os.paidAmount === 0 && (
                                  <button
                                    onClick={() => { setCancelTarget({ id: os.id, orderNumber: os.orderNumber }); setCancelReason(""); setCancelError(null) }}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
          <div className="bg-background border border-border rounded-xl shadow-xl w-full max-w-md mx-4 p-6 space-y-4">
            <div className="flex items-center justify-between">
              <div>
                <h2 className="text-sm font-semibold">Anular Orden de Servicio</h2>
