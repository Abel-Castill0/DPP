...
import type { PurchaseOrderDetail } from "@/lib/data/purchase-orders"
...
export function EditPurchaseOrderForm({ order, suppliers, items }: Props) {
  const proveedores = suppliers.filter((s) => s.supplierType === "PROVEEDOR_INSUMO")
  const insumos = items.filter((it) => it.itemType === "INSUMO")
    ...
    setLines((prev) =>
      prev.map((l) => {
        ...
        if (field === "description") {
          const found = insumos.find((i) => i.name === value)
        ...
        {!canEditFinancial && (
          <div className="flex items-start gap-3 p-3 rounded-lg bg-amber-50 border border-amber-200">
            ...
            <p className="text-xs text-amber-800">
              Esta orden ya fue enviada a caja. Solo se pueden editar fechas y observaciones.
              El proveedor, ítems y total están bloqueados para mantener la consistencia financiera.
            </p>
          </div>
        )}
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-xs">
                ...
                <tfoot className="border-t-2 border-border bg-muted/20">
                  <tr>
                    <td colSpan={canEditFinancial ? 4 : 4} className="px-4 py-3 text-xs font-semibold text-muted-foreground text-right">
                      Total general
                    </td>
