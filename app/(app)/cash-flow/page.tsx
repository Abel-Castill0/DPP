...
export default function CashFlowPage() {
  ...
  const filtered = demoCashMovements.filter((m) => {
    const matchSearch =
      !search ||
    ...
    const matchStatus =
      statusFilter === "all" || m.operationStatus === statusFilter
    return matchSearch && matchType && matchStatus
  })

  const totalIngresos = filtered
    .filter((m) => m.type === "INGRESO")
    .reduce((acc, m) => acc + m.abono, 0)
                    ...
                    {filtered.map((m) => (
                      ...
                      >
                        ...
                        <td className="px-4 py-3">
                          <TypeBadge type={m.type as "INGRESO" | "EGRESO"} />
                        </td>
                        <td className="px-4 py-3">
                          {m.origin === "ORDEN_COMPRA" && (
                            <span className="inline-block px-2 py-0.5 rounded-full text-[10px] font-medium bg-blue-100 text-blue-700">OC</span>
                          )}
                          {m.origin === "ORDEN_SERVICIO" && (
                            <span className="inline-block px-2 py-0.5 rounded-full text-[10px] font-medium bg-purple-100 text-purple-700">OS</span>
                          )}
                          {m.origin === "MANUAL" && (
                            <span className="inline-block px-2 py-0.5 rounded-full text-[10px] font-medium bg-muted text-muted-foreground">Manual</span>
                          )}
                        </td>
